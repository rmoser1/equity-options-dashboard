...
class Config:
    ...
    option_avg_batch_size = 2
    option_rate_limit_max_attempts = 4
    option_rate_limit_backoff_seconds = 60.0
    option_rate_limit_backoff_multiplier = 2.0
...
def test_app_wires_dependencies(monkeypatch):
    ...
    assert app.options_pipeline.kwargs["avg_batch_size"] == 2
    assert app.options_pipeline.kwargs["rate_limit_max_attempts"] == 4
    assert app.options_pipeline.kwargs["rate_limit_backoff_seconds"] == 60.0
    assert app.options_pipeline.kwargs["rate_limit_backoff_multiplier"] == 2.0
