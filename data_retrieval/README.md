...
| `RUN_OCC_PIPELINE` | `true` | Whether to refresh OCC underlyings before yfinance loads. |
| `OPTION_AVG_BATCH_SIZE` | `2` | Average randomized symbol batch size for the options pipeline. |
| `OPTION_RATE_LIMIT_MAX_ATTEMPTS` | `4` | Total attempts for a Yahoo option request that hits a rate limit. |
| `OPTION_RATE_LIMIT_BACKOFF_SECONDS` | `60.0` | Initial sleep before retrying a rate-limited Yahoo option request. |
| `OPTION_RATE_LIMIT_BACKOFF_MULTIPLIER` | `2.0` | Multiplier applied after each rate-limit retry sleep. |
...
The yfinance package is an unofficial Yahoo Finance wrapper and Yahoo does not
publish a stable request quota for this use. The options pipeline therefore uses
small configurable batches and exponential retry/backoff when Yahoo returns a
rate-limit response.
...
- The options pipeline uses smaller default batches than the stock-info and
  historical pipelines because one symbol can require many option-chain
  expiration requests.
...
- The dashboard parquet step writes one dataset at a time instead of holding all
  raw inputs and all transformed outputs in memory together.
