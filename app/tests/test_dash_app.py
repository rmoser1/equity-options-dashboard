...
def test_option_filters_use_static_loaded_data(app):
    """Verify option filter controls use the full loaded option universe."""
    expiration_dropdown = _component_by_id(app.layout, "SELECTOR_ExpirationDateDropdown")
    relative_strike_slider = _component_by_id(
        app.layout,
        "SELECTOR_RelativeStrikePriceRangeSlider",
    )
    ...
    assert [value.strftime("%Y-%m-%d") for value in expiration_dropdown.value] == [
        "2026-07-17",
        "2026-08-21",
    ]
