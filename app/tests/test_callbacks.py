...
from modules.dashboard_filter_state import (
    ...
    derive_option_filter_controls,
    option_symbols,
    valid_expiration_value,
    valid_relative_strike_value,
)
...
def test_option_filter_controls_follow_selected_symbols_and_type(app):
    """Verify option filter ranges and expirations are derived from data."""
    callback = _callback_by_name(app, "option_filter_controls_callback")

    result = callback(
        "SPY",
        ["MSFT"],
        "CALL",
        ["2026-08-21T00:00:00", "2030-01-01T00:00:00"],
        [0.5, 0.975],
    )

    assert result["expiration_options"] == [
        {"label": "2026-07-17", "value": "2026-07-17T00:00:00"},
        {"label": "2026-08-21", "value": "2026-08-21T00:00:00"},
    ]
    assert result["expiration_value"] == ["2026-08-21T00:00:00"]
    assert result["relative_strike_min"] == 0.9615
    assert result["relative_strike_max"] == 0.9808
    assert result["relative_strike_value"] == [0.9615, 0.975]


def test_dashboard_filter_state_helper_derives_controls(dashboard_parquet_dir):
    """Verify option-filter state helper can be tested outside Dash."""
    options = pd.read_parquet(dashboard_parquet_dir / "options_last.parquet")

    result = derive_option_filter_controls(
        options,
        "SPY",
        ["MSFT"],
        "CALL",
...
def test_dashboard_filter_state_helper_returns_empty_controls_without_matches(
    dashboard_parquet_dir,
):
    """Verify option-filter controls reset when selected data has no matches."""
    options = pd.read_parquet(dashboard_parquet_dir / "options_last.parquet")
    ...
    assert result.expiration_options == []
    assert result.expiration_value == []
    assert result.relative_strike_min == 0
    assert result.relative_strike_max == 1
    assert result.relative_strike_value == [0, 1]


def test_dashboard_filter_state_helpers_normalize_invalid_values():
    """Verify option filter helpers dedupe symbols and clamp stale values."""
    available_expirations = [
        "2026-07-17T00:00:00",
        "2026-08-21T00:00:00",
    ]

    assert option_symbols("SPY", ["MSFT", "SPY"]) == ["MSFT", "SPY"]
    assert valid_expiration_value(["2030-01-01"], available_expirations) == (
        available_expirations
    )
    assert valid_relative_strike_value([1.4, 1.5], 0.95, 0.99) == [0.95, 0.99]
    assert default_relative_strike_range([0.95, 0.99]) == [0.95, 0.99]
