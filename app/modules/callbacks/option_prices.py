...
from modules.dashboard_filter_state import DEFAULT_SINGLE_STOCK, derive_option_filter_controls
...
def register_option_price_callbacks(
    ...
    _register_option_filter_controls_callback(app, components, options)
...
def _register_option_filter_controls_callback(
    app: Dash,
    components: ComponentGroups,
    options: pd.DataFrame,
) -> None:
    """Register callbacks that constrain option filter controls to real data."""

    @app.callback(
        output=dict(
            expiration_options=Output(
                components.filters.expiration_dates,
                "options",
            ),
            expiration_value=Output(
                components.filters.expiration_dates,
                "value",
            ),
            relative_strike_min=Output(
                components.filters.relative_strike_price_range,
                "min",
            ),
            relative_strike_max=Output(
                components.filters.relative_strike_price_range,
                "max",
            ),
            relative_strike_value=Output(
                components.filters.relative_strike_price_range,
                "value",
            ),
        ),
        ...
        state=dict(
            current_expirations=State(components.filters.expiration_dates, "value"),
            current_relative_strikes=State(
                components.filters.relative_strike_price_range,
                "value",
            ),
        ),
    )
    def option_filter_controls_callback(
        selected_stock,
        selected_stocks,
        direction,
        current_expirations,
        ...
        controls = derive_option_filter_controls(
            options,
            selected_stock,
            selected_stocks,
            direction,
        ...
        return dict(
            expiration_options=controls.expiration_options,
            expiration_value=controls.expiration_value,
            relative_strike_min=controls.relative_strike_min,
            relative_strike_max=controls.relative_strike_max,
            relative_strike_value=controls.relative_strike_value,
        )
